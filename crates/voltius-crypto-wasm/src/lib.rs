...
pub fn aes_gcm_encrypt(key: &[u8], plaintext: &[u8]) -> Result<Vec<u8>, JsError> {
    ...
    voltius_crypto::aes_gcm_encrypt(k, plaintext).map_err(|e| JsError::new(&e))
}

#[wasm_bindgen]
pub fn aes_gcm_decrypt(key: &[u8], data: &[u8]) -> Result<Vec<u8>, JsError> {
    let k: &[u8; 32] = key.try_into().map_err(|_| JsError::new("key must be 32 bytes"))?;
    voltius_crypto::aes_gcm_decrypt(k, data).map_err(|e| JsError::new(&e))
}
...
#[wasm_bindgen]
pub fn wrap_user_secrets(kek: &[u8], dek: &[u8], x25519_private: &[u8]) -> Result<Vec<u8>, JsError> {
    let k: &[u8; 32] = kek.try_into().map_err(|_| JsError::new("kek must be 32 bytes"))?;
    let d: &[u8; 32] = dek.try_into().map_err(|_| JsError::new("dek must be 32 bytes"))?;
    let x: &[u8; 32] = x25519_private.try_into().map_err(|_| JsError::new("x25519_private must be 32 bytes"))?;
...
pub fn unwrap_user_secrets_x25519(kek: &[u8], wrapped: &[u8]) -> Result<Vec<u8>, JsError> {
    let k: &[u8; 32] = kek.try_into().map_err(|_| JsError::new("kek must be 32 bytes"))?;
    let (_, x25519_private) = voltius_crypto::unwrap_user_secrets(k, wrapped).map_err(|e| JsError::new(&e))?;
