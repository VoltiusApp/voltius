...
/// Derive master_key from password + account_id using Argon2id.
...
/// Derive auth_key and enc_key from master_key using HKDF-SHA256.
...
/// Full key derivation: password + account_id → { auth_key, enc_key }.
...
/// Generate X25519 keypair for future shared vaults.
...
pub struct Keypair {
    ...
    #[allow(dead_code)]
    pub private_key_bytes: Vec<u8>,      // raw bytes (to be encrypted with enc_key and stored in Stronghold)
}
