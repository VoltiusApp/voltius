...
function buildSelectionActions(files: FileEntry[], ctx: SelectionActionsCtx): ContextMenuItem[] {
  ...
  if (ctx.canTransferToTarget && files.length > 0) {
    ...
    items.push({ label: `Copy ${label} to target`, icon: "lucide:copy", onClick: () => ctx.onTransferToTarget?.(files) });
    if (!ctx.isLocal && ctx.sftpId) {
      const sid = ctx.sftpId;
      items.push({
        label: `Move ${label} to target`, icon: "lucide:scissors",
        onClick: async () => {
  ...
  // Rename / Delete
  if (single) items.push({ label: `Rename "${single.name}"`, icon: "lucide:pencil", onClick: () => ctx.onStartRename(single) });
  if (files.length > 0) {
    items.push({
      label: files.length === 1 ? `Delete "${files[0].name}"` : `Delete ${files.length} items`,
...
function ColumnHeaders({ sortCol, sortDir, isLocal, colWidths, visibleCols, onSort, onResize }: {
  ...
  return (
    <div className="flex items-center gap-2 px-3 py-1 shrink-0 border-b border-b-[var(--t-border)]" style={{ background: "var(--t-bg-card)" }}>
      ...
      <button
        onClick={() => onSort("name")}
        className="flex-1 min-w-0 relative flex items-center gap-0.5 text-[11px] font-medium transition-colors"
      ...
      {(["size", "modified", ...(!isLocal ? ["permissions"] : [])] as (keyof ColWidths)[]).filter((col) => visibleCols[col]).map((col) => {
        ...
        return (
          <button
            ...
            className="relative flex items-center justify-end gap-0.5 text-[11px] font-medium transition-colors shrink-0"
...
function VirtualFileList({
  ...
  const inlineCreateRow = (creatingFolder || creatingFile) && (
    <div className="flex items-center gap-2 m-1.5 px-2 py-0.5 mx-1 rounded border border-[var(--t-accent)]">
      <Icon icon={creatingFolder ? "lucide:folder" : "lucide:file"} width={13} className="shrink-0" style={{ color: creatingFolder ? "#f0c050" : "var(--t-text-dim)" }} />
      <input
        autoFocus
        value={newItemName}
          ...
          if (renaming === file.path) {
            return (
              <div key={file.path} style={itemStyle}>
                <div className="flex items-center gap-2 m-1.5 px-2 py-0.5 mx-1 rounded border border-[var(--t-accent)]">
                  <Icon icon={file.isDir ? "lucide:folder" : "lucide:file"} width={13} className="shrink-0" style={{ color: file.isDir ? "#f0c050" : "var(--t-text-dim)" }} />
                  <input
                    autoFocus
                    value={renameVal}
                    ...
                    className="flex-1 text-xs bg-transparent outline-none text-[var(--t-text-primary)]"
                  />
                </div>
              </div>
            );
          ...
          return (
            ...
            >
              <FileRow
                ...
                onDoubleClick={() => { if (file.isDir) onNavigate(file.path); else if (!selectionActionsCtx.isLocal) selectionActionsCtx.onStartRename(file); }}
                ...
                onDragStart={(e) => {
                  ...
                  ghost.style.cssText = "position:fixed;top:-9999px;left:-9999px;display:flex;align-items:center;gap:6px;padding:6px 10px;border-radius:8px;font-size:12px;font-family:inherit;white-space:nowrap;box-shadow:0 4px 16px rgba(0,0,0,0.5);pointer-events:none;";
...
function FileRow({ file, isSelected, isDragHover, isLocal, colWidths, visibleCols, selectableId, onClick, onDoubleClick, contextActions, onDragStart, onDragEnd }: {
  ...
  return (
    <div
      ...
      className="flex items-center gap-2 m-1.5 px-2 py-0.5 mx-1 rounded transition-colors cursor-default select-none relative"
      style={{
    ...
    >
      <Icon
        ...
        width={13} className="shrink-0"
        style={{ color: file.isDir ? "#f0c050" : file.isSymlink ? "var(--t-accent)" : "var(--t-text-dim)" }}
      />
      <span className="flex-1 text-xs truncate text-[var(--t-text-primary)] min-w-0">{file.name}</span>
      {visibleCols.size && (
