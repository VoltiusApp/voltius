...
export default function HostsPage() {
  const { connections, loadConnections, saveConnection, updateConnection, deleteConnection, renameTag, deleteTag } =
    useConnectionStore();
  ...
  useEffect(() => {
    ...
    if (homePendingAction.action === "create") {
      ...
      if (conn) { setEditing(conn); setShowForm(true); setEditingFolderId(null); }
    }
  ...
  const { focusedId, setFocusedId } = useListKeyNav({
    ...
    onEdit: (id) => {
      const conn = connections.find((c) => c.id === id);
      if (conn) { selectSingle(conn.id); setEditing(conn); setShowForm(true); setEditingFolderId(null); }
    },
    onDuplicate: (id) => {
      const conn = connections.find((c) => c.id === id);
    ...
    onEscape: () => {
      if (showForm || editingFolderId) { setShowForm(false); setEditing(null); setEditingFolderId(null); }
      else setSelection([]);
    },
    ...
    requestCascade({
      ...
      execute: async () => {
        try {
          ...
          const newFolder = await saveFolder({ name: `${folder.name} (copy)`, object_type: folder.object_type, parent_folder_id: folder.parent_folder_id, vault_id: vaultId });
        ...
        <>
          ...
          {showForm && (
            <ConnectionForm
              ...
              onDelete={editing ? () => { deleteConnection(editing.id); setShowForm(false); setEditing(null); } : undefined}
    ...
    >
        <div>
          <HomeToolbar
            ...
            onCreateFolder={() => void saveFolder({ name: "New Folder", object_type: "connection", parent_folder_id: activeFolderId ?? undefined, vault_id: defaultVaultId }).then((f) => { setShowForm(false); setEditing(null); setEditingFolderId(f.id); })}
                    ...
                    {canCreateFolder && <button
                      ...
                      onClick={() =>
                        saveFolder({ name: "New Folder", object_type: "connection", parent_folder_id: activeFolderId ?? undefined, vault_id: defaultVaultId }).then((f) => {
                          setShowForm(false); setEditing(null); setEditingFolderId(f.id);
                        })
                      }
                    >
                    ...
                    {visibleFolders.map((folder) => {
                      ...
                      return (
                        <FolderCard
                          ...
                          onEdit={() => { setShowForm(false); setEditing(null); setEditingFolderId(folder.id); }}
                    ...
                    {pinnedHosts.map((conn) => {
                      ...
                      return (
                        <HostCard
                          ...
                          onEdit={(c) => { selectSingle(c.id); setEditing(c); setShowForm(true); setEditingFolderId(null); }}
                  ...
                  <div className="flex items-center justify-between mb-3">
                    ...
                    {activeFolderId && (
                      <button
                        ...
                        onClick={() => { if (canCreate) { setEditing(null); setShowForm(true); setEditingFolderId(null); } }}
                        disabled={!canCreate}
              ...
              {activeFolderId && filtered.length === 0 && !showForm && (
                <div className="flex flex-col items-center justify-center py-12 gap-3">
                  ...
                  <button
                    ...
                    onClick={() => { setEditing(null); setShowForm(true); setEditingFolderId(null); }}
                  >
                    <Icon icon="lucide:plus" width={12} />
                    Add Host
                  </button>
      ...
      {bgMenuPos && (
        <ContextMenu
          ...
          onClose={closeBgMenu}
          items={[
            ...(canCreate ? [{ label: "New Host", icon: "lucide:server", onClick: () => { setEditing(null); setShowForm(true); setEditingFolderId(null); } } as const] : []),
            ...(canCreateFolder ? [{ label: "New Folder", icon: "lucide:folder-plus", onClick: () => void saveFolder({ name: "New Folder", object_type: "connection", parent_folder_id: activeFolderId ?? undefined, vault_id: defaultVaultId }).then((f) => { setShowForm(false); setEditing(null); setEditingFolderId(f.id); }) } as const] : []),
            ...bgContributions,
          ]}
        />
      )}
