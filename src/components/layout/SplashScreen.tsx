...
export default function SplashScreen({ onReady }: Props) {
  ...
  const finishLoading = async () => {
    ...
    isServerMode().then((server) => {
      if (!server) return;
      // After a switchToAccount reload, use replace-mode sync so the previous
      // account's local files are never merged into the new account's cloud data.
      ...
      (useReplace ? syncOnLoginReplace() : syncOnLogin()).catch(() => {});
      startRealtimeSync();
    });
