...
export default function TitleBar() {
  ...
  return (
    ...
    >
      ...
      {/* Sync indicator */}
      {(syncState.cloudActive || syncState.status !== "idle") && (
        <SyncIndicator status={syncState.status} lastSync={syncState.lastSync} error={syncState.error} />
      )}
      ...
      {showTerminal && !isActiveSessionMultiplayer && (
        <div className="flex items-center px-1 shrink-0">
          <button
            ...
            title={isActiveSessionSharing ? "Currently sharing" : accountMode !== "server" ? "Sign in to share your terminal" : "Share terminal with team"}
            onMouseEnter={(e) => {
          ...
          >
            <Icon icon={isActiveSessionSharing ? "lucide:radio" : "lucide:share-2"} width={13} />
...
function SyncIndicator({ status, lastSync, error }: { status: SyncStatus; lastSync: Date | null; error: string | null }) {
  const { createRipple, rippleEls } = useRipple();
  const icon =
  ...
  const title =
    ...
    status === "error"   ? `Sync error: ${error ?? "unknown"} — click to retry` :
    status === "offline" ? "Offline" :
                           "Sync";

  const clickable = status !== "syncing";

  return (
    <div className="flex items-center px-1 shrink-0">
      <button
        onClick={() => { if (clickable) syncNow().catch(() => {}); }}
        onMouseDown={clickable ? createRipple : undefined}
        className={`flex items-center justify-center w-8 h-8 rounded-xl transition-colors relative overflow-hidden ${clickable ? "cursor-pointer" : "cursor-default"}`}
        style={{ color }}
        title={title}
        onMouseEnter={(e) => {
          if (clickable) {
