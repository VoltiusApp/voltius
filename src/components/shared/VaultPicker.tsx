...
export function VaultPicker({
  ...
  const { teams, membersByTeam, loadMembers, loadTeams } = useTeamStore();
  ...
  useEffect(() => {
    teams.forEach((t) => { if (!membersByTeam[t.id]) loadMembers(t.id).catch(() => {}); });
  }, [teams, membersByTeam, loadMembers]);

  const myRoleIn = (teamId: string): string => {
    const members = membersByTeam[teamId];
    if (!members) return "";
    return members.find((m) => m.user_id === myUserId)?.role ?? "";
  };

  const canWrite = (vId: string) => {
    if (vId === "personal") return true;
    const role = myRoleIn(vId);
        ...
        <>
          ...
          >
            {allVaults.map((v) => {
              ...
              const role = v.team ? myRoleIn(v.id) : "";
