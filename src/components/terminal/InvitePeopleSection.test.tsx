...
type SectionProps = Parameters<typeof InvitePeopleSection>[0];

/**
 * Mirrors ShareMenu's ownership of the "invited during this menu session" set. It
 * lives in the parent because it has to outlive any single InvitePeopleSection
 * instance: the first invite on an unshared terminal flips the setup view to the
 * active view, remounting this component.
 */
function Harness({ onInvite, ...props }: Omit<SectionProps, "invitedThisSession">) {
  const [invited, setInvited] = useState<ReadonlySet<string>>(new Set());
  const handleInvite = async (member: Parameters<SectionProps["onInvite"]>[0]) => {
    await onInvite(member);
    setInvited((prev) => new Set(prev).add(member.user_id));
  };
  return <InvitePeopleSection {...props} invitedThisSession={invited} onInvite={handleInvite} />;
}
...
test("marks a covered teammate as having access and does not call onInvite", async () => {
  ...
  render(
    <Harness session={{ vaultIds: ["t1"], participantIds: [], invitedIds: [] }} {...HIGH_CAP} onInvite={onInvite} />,
  );
...
test("disables the row while an invite is in flight and shows Invited after", async () => {
  ...
  render(
    <Harness session={{ vaultIds: [], participantIds: [], invitedIds: [] }} {...HIGH_CAP} onInvite={onInvite} />,
  );
  const row = (await screen.findByRole("button", { name: /alice/i })) as HTMLButtonElement;
  await userEvent.click(row);
  expect(row.disabled).toBe(true);
...
test("renders nothing while the roster is empty (no header flash)", async () => {
  ...
  const { container } = render(
    <Harness session={{ vaultIds: [], participantIds: [], invitedIds: [] }} {...HIGH_CAP} onInvite={vi.fn()} />,
  );
