...
// Dedicated file: the seeded manifest here has an unparseable version, which needs
// its own fixture (see marketplaceStore.supersede.test.ts's file-level cache note).
const h = vi.hoisted(() => ({
  invoke: vi.fn(),
  loadPlugin: vi.fn(),
  unloadPlugin: vi.fn(),
...
const SEEDED_MANIFEST = { id: "plugin-docker", name: "Docker", version: "garbage", permissions: ["docker:read"] };
...
test("malformed seeded version: left alone rather than defaulting to supersede", async () => {
