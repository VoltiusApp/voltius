...
const FIRST_PARTY_SOURCE: MarketplaceSource = {
  ...
  url: "https://raw.githubusercontent.com/voltius/marketplace/main/plugins.json",
  enabled: true,
  deletable: false,
};
  ...
  async fetchCatalog() {
    ...
    for (const source of sources.filter((s) => s.enabled)) {
      try {
        ...
        const plugins = (data.plugins ?? []).map((p) => ({ ...p, sourceId: source.id }));
  ...
  async installPlugin(plugin: MarketplacePlugin) {
    ...
    try {
      ...
      const [manifestText, jsText] = await Promise.all([
        fetch(`${base}/manifest.json`).then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.text(); }),
        fetch(`${base}/index.js`).then((r) => { if (!r.ok) throw new Error(`HTTP ${r.status}`); return r.text(); }),
      ]);
